/* ------------------------------------------------------------------ */
/*  Face Composite v24 ÃÂ¢ÃÂÃÂ restores original face onto VTO result        */
/*                                                                      */
...
async function detectFace(img: HTMLImageElement): Promise<FaceBox> {
  if ("FaceDetector" in window) {
    try {
      // @ts-ignore ÃÂ¢ÃÂÃÂ FaceDetector is a Chrome-only API
...
function applyColorCorrection(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      // Calculate distance from face center ÃÂ¢ÃÂÃÂ apply stronger correction near face
...
function createFeatheredMask(
  ...
  // v24: Very tight mask ÃÂ¢ÃÂÃÂ face only, NO chin/neck extension
  // v23 still bled into collar/chest shifting garment color
  const neckExtend = 0;  // was 0.10 ÃÂ¢ÃÂÃÂ completely removed to protect garment collar
  const cx = box.x + box.width / 2;
  ...
  // Sharper gradient ÃÂ¢ÃÂÃÂ solid core larger, faster falloff at edges
...
export async function compositeFaceOntoVTO(
  ...
  // v24: Much smaller radius ÃÂ¢ÃÂÃÂ only correct pixels very close to face center
  // v26: Disabled color correction entirely to prevent collar color bleed
  // // v23 radius of 0.3 still reached collar/chest and shifted garment color
  // applyColorCorrection(
  // origCtx, outW, outH,
  // origStats, vtoStats,
  // vtoFace,
  // Math.max(vtoFace.width, vtoFace.height) * 0.10  // v24: minimal ÃÂ¢ÃÂÃÂ face only
  // );

  // v24: Very tight mask ÃÂ¢ÃÂÃÂ no neck extension, sharper falloff

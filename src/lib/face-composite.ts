/* ------------------------------------------------------------------ */
/*  Face Composite v24 Ã¢ÂÂ restores original face onto VTO result        */
/*                                                                      */
...
async function detectFace(img: HTMLImageElement): Promise<FaceBox> {
  if ("FaceDetector" in window) {
    try {
      // @ts-ignore Ã¢ÂÂ FaceDetector is a Chrome-only API
...
function applyColorCorrection(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      // Calculate distance from face center Ã¢ÂÂ apply stronger correction near face
...
function createFeatheredMask(
  ...
  // v24: Very tight mask Ã¢ÂÂ face only, NO chin/neck extension
  // v23 still bled into collar/chest shifting garment color
  const neckExtend = 0;  // was 0.10 Ã¢ÂÂ completely removed to protect garment collar
  const cx = box.x + box.width / 2;
  ...
  // Sharper gradient Ã¢ÂÂ solid core larger, faster falloff at edges
  ...
  const bottomFadeStart = box.y + box.height * 0.50;
  const bottomFadeEnd = box.y + box.height * 0.65;
...
export async function compositeFaceOntoVTO(
  ...
  // v24: Much smaller radius Ã¢ÂÂ only correct pixels very close to face center
